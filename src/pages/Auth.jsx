...
const Auth = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 font-sans bg-primary-bgColor">
      <div className={`relative overflow-hidden bg-primary-dark rounded-3xl shadow-xl w-full max-w-[768px] min-h-[480px] ${isActive ? 'active' : ''}`}>
        <div className="absolute top-0 w-full h-full transition-all ease-in-out duration-600">
          ...
          <div className={`absolute top-0 h-full transition-all duration-600 ease-in-out ${isActive ? 'translate-x-0 opacity-100 z-20' : 'translate-x-full opacity-0 z-10'} w-1/2 right-0`}>
            <form onSubmit={(e) => handleSubmit(e, 'signup')} className="flex flex-col items-center justify-center h-full px-6 bg-primary-dark sm:px-10">
              <h1 className="mb-4 font-serif text-2xl font-bold text-primary-white">Create Account</h1>
              ...
              <span className="my-4 text-sm text-center text-primary-white">or use your email for registration</span>
          ...
          <div className={`absolute top-0 h-full transition-all duration-600 ease-in-out ${isActive ? '-translate-x-full opacity-0' : 'translate-x-0 opacity-100 z-20'} left-0 w-1/2`}>
            <form onSubmit={(e) => handleSubmit(e, 'signin')} className="flex flex-col items-center justify-center h-full px-6 bg-primary-dark sm:px-10">
              <h1 className="mb-4 font-serif text-2xl font-bold text-primary-white">Sign In</h1>
              {/* <div className="flex mb-5 space-x-3">
                {[<FaGooglePlusG />, <FaGithub />, <FaLinkedinIn />].map((icon, index) => (
                  <a key={index} href="#" className="flex items-center justify-center w-10 h-10 transition-colors duration-300 rounded-lg bg-primary-yellow text-primary-dark hover:bg-primary-light hover:text-primary-dark">
                    {icon}
              ...
              <span className="my-4 text-sm text-center text-primary-white">or use your email password</span>
              <input type="email" placeholder="Email" className="w-full px-4 py-2 mb-2 text-sm border-none rounded-lg outline-none bg-primary-grey text-primary-white" />
              <input type="password" placeholder="Password" className="w-full px-4 py-2 mb-2 text-sm border-none rounded-lg outline-none bg-primary-grey text-primary-white" />
        ...
        <div className={`absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-all duration-600 ease-in-out ${isActive ? '-translate-x-full rounded-r-[150px]' : 'rounded-l-[150px]'} z-50`}>
          <div className={`bg-gradient-to-r from-primary-yellow to-primary-light text-primary-dark relative -left-full h-full w-[200%] ${isActive ? 'translate-x-1/2' : 'translate-x-0'} transition-all duration-600 ease-in-out`}>
            <div className={`absolute flex flex-col items-center justify-center px-8 text-center top-0 h-full w-1/2 ${isActive ? 'translate-x-0' : '-translate-x-[200%]'} transition-all duration-600 ease-in-out`}>
              <h1 className="mb-2 font-serif text-2xl font-bold">Welcome Back!</h1>
              <p className="mb-4 text-sm">Enter your personal details to use all of site features</p>
            ...
            <div className={`absolute flex flex-col items-center justify-center px-8 text-center top-0 right-0 h-full w-1/2 ${isActive ? 'translate-x-[200%]' : 'translate-x-0'} transition-all duration-600 ease-in-out`}>
              <h1 className="mb-2 font-serif text-2xl font-bold">Hello, Friend!</h1>
              <p className="mb-4 text-sm">Register with your personal details to use all of site features</p>
